...
router.get("/event/:event/id/:department_id", function(req, res) {
  Category.find({ name: req.params.event }, (err, doc) => {
    Event.find(
      { department_id: req.params.department_id, category_id: doc[0]._id },
      (err, docs) => {
        res.send(docs);
      }
    );
...
router.get("/", function(req, res, next) {
  Event.find()
    .skip((page - 1) * config.pagination.perPage)
...
//getWorkshops route added by Siddeshwar on 27/12/19 (in order not to tamper with existing route)

router.get("/getWorkshops", (req, res) => {
  var docs = [];
  Event.find({ category_id: ObjectId("5df8665020e07e47184f93ab") })
    .populate("department_id")
    .exec((err, docs) => {
      res.json(docs);
    });
